...
CoMoMo.CoMoMo <- function(models, data = NULL, weight = NULL, Dxt = NULL, Ext = NULL, ages.fit = NULL, years.fit = NULL, ages = NULL, years = NULL, h = NULL) {
  ...
  prediction <- fitCoMoMo(models = models, data = data, ages.fit = ages.fit, years.fit = years.fit, h = h)
   ...
   {
     mortfor <- prediction%>%dplyr::left_join(weight$weights)%>%dplyr::mutate(forrates = log(rate)*weights)%>%dplyr::group_by(ages, year, h)%>%dplyr::summarise(comb = exp(sum(forrates)))%>%dplyr::ungroup()%>%
     ...
     output <-list(comb.rates = na.omit(mortfor), single.rates = prediction, comb.method = weight$comb.method, mse = na.omit(mseDF))


     class(output) <- "CoMoMo.CoMoMo"
...

 {
   ...
   weightsDF <- (dplyr::bind_rows(lapply(rep(list(as.data.frame(tail(weight$weights, length(models)))), interval), function(x) x%>%dplyr::mutate(model = names(models))))%>%dplyr::mutate(h = rep(1:interval + start.horizon, each=length(models))))[, c("h","weights","model")]
